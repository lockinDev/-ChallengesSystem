package com.devlockin.test.multiplication.challenge.helper;
...
import com.devlockin.multiplication.challenge.helper.ChallengeSolvedEvent;
...
class ChallengeEventPubTest {
	...
	public void sendsAttempt(boolean correct) {
		...
		ArgumentCaptor<ChallengeSolvedEvent> eventCaptor = ArgumentCaptor.forClass(ChallengeSolvedEvent.class);
	...
	private ChallengeAttempt createTestAttempt(boolean correct) {
		return new ChallengeAttempt(1L, new User(10L, "test"), 30, 40, correct ? 1200 : 1300, correct);
	}

	private ChallengeSolvedEvent solvedEvent(boolean correct) {
		return new ChallengeSolvedEvent(1L, correct, 30, 40, 10L, "test");
	}
}